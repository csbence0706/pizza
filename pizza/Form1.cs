...
namespace pizza
{
    public partial class PizzaForm : Form
    {
        ...
        private List<Pizza> pizzakLista = new List<Pizza>();
        private List<CheckBox> jNegyzetek = new List<CheckBox>();
        ...
        private void Feldolgoz(string adat)
        {
            ...
            pizzakLista.Add(new Pizza(adatok[0], int.Parse(adatok[1]), int.Parse(adatok[2])));
        }
        ...
        private void ValasztekFeltoltes()
        {
            ...
            for (int i = 0; i < pizzakLista.Count; i++)
            {
                ...
                radioButton.Text = pizzakLista[i].ArKicsi.ToString();
        ...
        private void btnSzamol_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < jNegyzetek.Count; i++)
            {
                if (jNegyzetek[i].Checked)
                {
                    kiJelol = true;
                    try
                    {
                        ...
                        {
                            ar = pizzakLista[i].ArKicsi;
                        }
                        else if (rdBtnNagyArak[i].Checked)
                        {
                            ar = pizzakLista[i].ArNagy;
                        }
                    ...
                    {
                        MessageBox.Show("Nem választottál méretet", "Hiba");
                        MessageBox.Show("Nem választottál méretet", "Hiba");
                    }
                    catch (Exception)
                    {
                        MessageBox.Show("Hibásan adtál meg darabszámot", "Hiba");
        ...
        private void checkBox_ChecledChanged(object sender, EventArgs e)
        {
            ...
            int index = jNegyzetek.IndexOf(valasztoGomb);
